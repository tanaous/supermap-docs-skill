...
<body class="rh-BODY-wrapper">
  ...
  <p class="myNormal"> 针对以 CSV、Excel、GeoJSON 格式存储的结构化数据，SuperMap iPortal 提供了数据上传、下载、导入数据到 PostgreSQL / Yukon 数据库、获取字段统计信息、获取数据 mvt 瓦片的能力。当前，MapStudio WebApp 提供了对结构化数据能力的支持，在使用之前，iPortal 管理员需配置 PostgreSQL/Yukon 数据库作为结构化数据存储。</p>
  <h2>使用 PostgreSQL 数据库作为结构化数据存储</h2>
  <p class="myNormal">在 MapStudio WebApp 中上传的结构化数据支持存储到 PostgreSQL 的空间数据库中。在此之前，您需要先完成 <a class="contentpagehyperlink" href="../../../Subject_introduce/Security/otherSecurity/PostgreSQL_install/PostgreSQL_install_config.htm">PostgreSQL 数据库的安装与配置</a>，并在此基础上，<a class="contentpagehyperlink" href="PostGISinstall.htm">安装与配置对应版本的 PostGIS</a>，扩展 PostgreSQL 存储与管理空间数据的能力 。</p>
  <p class="myNormal">在完成数据库安装与配置后，以管理员身份登录门户首页，<span>依次点击“管理”&gt;“站点配置”&gt;“数据存储”，选择“结构化数据存储”选项卡，进入结构化数据配置页面：</span></p>
  <ul class="SymbolList2" style="list-style: circle;">
    <li class="SymbolList2"><span style="font-weight: bold;">服务地址</span>：填写 PostgreSQL 中用于存储结构化数据的空间数据库地址，如：127.0.0.1:5432/postgis_24_sample。</li>
    <li class="SymbolList2"><span style="font-weight: bold;">用户名</span>：填写用于登录数据库的用户名。</li>
    <li class="SymbolList2"><span style="font-weight: bold;">密码</span>：填写用于登录数据库的密码。</li>
  </ul>
  <p class="myNormal">填写完成后，点击保存即可。</p>
  <p class="myNormal"> </p>
  <h2>使用 Yukon 数据库作为结构化数据存储</h2>
  <p class="myNormal">Yukon（禹贡）数据库，基于 openGauss 数据库扩展地理空间数据的存储和管理能力，提供专业的 GIS 功能，赋能传统关系型数据库。</p>
  <p class="myNormal">SuperMap iPortal 适配 Yukon 1.0.1 版本，在 MapStudio WebApp 中上传的结构化数据可以存储在 Yukon 的空间数据库中。在此之前，您需要先完成 <a class="contentpagehyperlink" href="http://yukon.supermap.io/1.0.1/installation.html" target="_blank">Yukon 数据库的安装与配置</a>。</p>
  <p class="myNormal">在完成数据库安装与配置后，以管理员身份登录门户首页，<span>依次点击“管理”&gt;“站点配置”&gt;“数据存储”，选择“结构化数据存储”选项卡，进入结构化数据配置页面：</span></p>
  <ul class="SymbolList2" style="list-style: circle;">
    <li class="SymbolList2"><span style="font-weight: bold;">服务地址</span>：填写 Yukon 中用于存储结构化数据的空间数据库地址，如：127.0.0.1:5432/yukon。</li>
    <li class="SymbolList2"><span style="font-weight: bold;">用户名</span>：填写用于登录数据库的用户名。</li>
    <li class="SymbolList2"><span style="font-weight: bold;">密码</span>：填写用于登录数据库的密码。</li>
  </ul>
  <p class="myNormal">填写完成后，点击保存即可。</p>
