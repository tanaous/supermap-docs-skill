...
## URI
[<datasetInterpolation_uri>](../interpolation.htm#URI)/idw[.<format>]
## 支持的方法
[GET](#GET 请求)、[POST](#POST 请求)、[HEAD](#HEAD 请求)
## 父资源
[datasetInterpolation](../interpolation.htm)
## 子资源
[interpolationIDWResult](interpolationIDWResult.htm)
## 介绍
反距离加权插值（Inverse Distance Weighted ，IDW）算法，通过计算附近区域离散点群的平均值来估算单元格的值，生成栅格数据集。 这是一种简单有效的数据内插方法，运算速度相对较快。反距离加权算法假设离预测点越近的值对预测点的影响越大，即预测某点的值时，其周围点的权值与距离预测点的距离成反比。
对该资源发送 POST 请求可以创建一个距离反比权重值插值分析结果资源。
支持的方法：
- [GET](#GET 请求)： html表述返回一个 POST 请求的表单，其他表述返回距离反比权重值插值分析结果集列表。
- [POST](#POST 请求)：创建一个反距离加权插值结果资源，相当于进行了一次插值分析。
- [HEAD](#HEAD 请求)：检查 interpolationIDW 资源是否存在，或权限是否可以访问 interpolationIDW 资源。
支持的表述格式：（rjson、json、jsonp、html、xml）。
## 资源层次
!\[](../../../../../../../assets/images/root/spatialanalyst/datasets/sp_ds_interpolationIDW.png)[ 
## HTTP 请求方法
...
### ](../../../../root.htm)GET 请求
反距离加权插值资源，通过该资源可以进行反距离加权插值操作。
html表述返回一个 POST 请求的表单，其他表述返回距离反比权重值插值分析结果集列表。
### POST 请求
创建一个反距离加权插值结果资源，相当于进行了一次插值分析。
#### 请求参数
执行 POST 请求时，请求体需包含如下参数：
| 名称 | 类型 | 含义 |
| --- | --- | --- |
| power | int | 距离权重计算的幂次，该值决定了权值下降的速度，幂次越大，随距离的增大权值下降越快，距离预测点越远的点的权值也越小。 理论上，参数值必须大于0，但是0.5到3之间时运算结果更合理，因此推荐值为0.5~3，默认值为2， |
...
| searchMode | [SearchMode](../../../../../../iServerJavadoc/com/supermap/services/components/commontypes/SearchMode.html) | 插值运算时，查找参与运算点的方式，支持固定点数查找、定长查找。 |
| expectedCount | int | 【固定点数查找】 设置待查找的点数，即参与差值运算的点数。默认为12。 |
| searchRadius | double | 【定长查找】 查找半径，即参与运算点的查找范围，与点数据集单位相同。计算某个位置的 Z 值时，会以该位置为圆心，以查找范围的值为半径，落在这个范围内的采样点都将参与运算。 |
...
#### 响应示例
对 interpolationIDW 资源执行 POST 请求，即对 http://localhost:8090/iserver/services/spatialanalyst-sample/restjsr/spatialanalyst/datasets/SamplesP@Interpolation/interpolation/idw.rjson 提交请求体如下：
{
"power": 2,
...
对 http://localhost:8090/iserver/services/spatialanalyst-sample/restjsr/spatialanalyst/datasets/SamplesP@Interpolation/interpolation/idw/mi92arb7_ced19c5194e54209bd9112871f34f7c9.rjson ,执行 GET 请求，即可获取反距离加权插值分析结果的具体信息。请参见 [interpolationIDWResult](interpolationIDWResult.htm)。
### HEAD 请求
返回跟 GET 请求一样的 HTTP 响应头，但是没有响应实体。可以在不必传输整个响应内容的情况下，获取包含在响应消息头中的元数据信息。元数据信息包括媒体类型，字符编码，压缩编码，实体内容长度等。
HEAD 请求可以用来判断 interpolationIDW 资源是否存在，或者客户端是否有权限访问 interpolationIDW 资源。通过对加.<format>的 URI 执行 HEAD 请求，还可以快速判断 interpolationIDW 资源是否支持<format>格式的表述。
## 请参见
- [datasetInterpolation](../interpolation.htm)、[interpolationIDWResult](interpolationIDWResult.htm)
