...
## URI
URI1：[<dataset_uri>](../dataset.htm#URI)/isoregion[.<format>]
URI2：[<dataset_uri>](../dataset.htm#URI)/isoregion/{zValuedFieldName}/{resolution}[.<format>]
## 支持的方法
[GET](#GET 请求)、[POST](#POST 请求)、[HEAD](#HEAD 请求)
## 父资源
[dataset](../dataset.htm)
## 子资源
[datasetIsoregionResult](../datasetIsolineResults/datasetIsolineResult.htm)
## 介绍
可以对栅格数据集和点数据集提取等值面，对栅格数据集提取等值面时，filterQueryParameter，zValueFieldName，resolution 三个参数是不需要的。同时，对点数据集提取等值面还可以使用 URI2，此时 zValueFile 和 resolution 在 URL 中传递。
支持的方法：
- [GET](#GET 请求)：html表述返回一个 POST 请求的表单，其他表述返回等值面提取结果集列表。
- [POST](#POST 请求)：创建一个提取等值面的结果资源，相当于进行了一次提取等值面的表面分析。
- [HEAD](#HEAD 请求)：检查 datasetIsoregionResults 资源是否存在，或权限是否可以访问 datasetIsoregionResults 资源。
支持的表述格式：rjson、json、html、xml。
## 资源层次
!\[](../../../../../../assets/images/root/spatialanalyst/datasets/sp_ds_dt_isoregionresults.png)[ 
## HTTP 请求方法
...
### ](../../../root.htm)GET 请求
html表述返回一个 POST 请求的表单，其他表述返回等值面提取结果集列表。
### POST 请求
对资源进行 POST 请求，传递相关参数后，就创建了一个表面分析结果资源，相当于进行了一次提取等值面的表面分析。
#### 请求参数
URI 中的参数如下：
| 名称 | 类型 | 含义 |
| --- | --- | --- |
...
URI1可用于对栅格和点数据集提取等值面，请求体参数如下：
其中，对栅格数据集提取等值面时，filterQueryParameter，zValueFieldName，resolution 三个参数是不需要的。
| 名称 | 类型 | 含义 |
| --- | --- | --- |
...
| zValuedFieldName | String | 用于提取操作的字段名称。提取等值面时，将使用该字段中的值，对点数据集进行插值分析，得到栅格数据集（中间结果），接着从栅格数据集提取等值线，最终由等值线构成等值面。 |
| resolution | double | 中间结果栅格数据集的分辨率，表示一个像元所代表的实地距离，其单位与源操作数据集单位相同。该值越小，表示中间结果栅格数据集的精度越高，从而所提取的等值面精度也越高。该值的取值范围有如下限制：
...
URI2用于对点数据集提取等值面，zValueFieldName 和 resolution 在 URL 中传递，请求体参数如下，
| 名称 | 类型 | 含义 |
| --- | --- | --- |
...
#### 响应示例
对 Isoregion 资源：http://supermapiserver:8090/iserver/services/spatialanalyst-sample/restjsr/spatialanalyst/datasets/SamplesP@Interpolation/isoregion，执行 POST 请求参数如下：
{
...
对资源：http://supermapiserver:8090/iserver/services/spatialanalyst-sample/restjsr/spatialanalyst/datasets/SamplesP%40Interpolation/isoregion/1，执行 GET 请求，即可获取分析结果的具体信息。请参见[datasetIsoregionResult](datasetIsoregionResult.htm)。
### HEAD 请求
返回跟 GET 请求一样的 HTTP 响应头，但是没有响应实体。可以在不必传输整个响应内容的情况下，获取包含在响应消息头中的元数据信息。元数据信息包括媒体类型，字符编码，压缩编码，实体内容长度等。
HEAD 请求可以用来判断 datasetIsoregionResults 资源是否存在，或者客户端是否有权限访问 datasetIsoregionResults 资源。通过对加.<format>的 URI 执行 HEAD 请求，还可以快速判断 datasetIsoregionResults 资源是否支持<format>格式的表述。
## 请参见
- [dataset](../dataset.htm)，[datasetIsoregionResult](../datasetIsolineResults/datasetIsolineResult.htm)
