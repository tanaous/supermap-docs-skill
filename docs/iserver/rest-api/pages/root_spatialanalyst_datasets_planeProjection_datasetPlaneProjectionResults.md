...
## URI
<[dataset_uri](../dataset.htm#URI)>/planeprojection[.<format>]
## 支持的方法
[GET](#GET 请求)、[POST](#POST 请求)、[HEAD](#HEAD 请求)
## 父资源
[dataset](../dataset.htm)
## 子资源
[datasetPlaneProjectionResult](datasetPlaneProjectionResult.htm)
## 介绍
模型数据集获取平面投影，该资源只针对模型数据集，获取指定模型数据集的平面投影结果。
对该资源发送POST请求，则可以创建一个平面投影分析结果资源。
支持的方法：
- [GET](#GET 请求)：返回一个进行POST请求的表单。
- [POST](#POST 请求)：创建一个平面投影分析结果资源。
- [HEAD](#HEAD 请求)：检查planeProjection资源是否存在，或权限是否可以访问planeProjection资源。
支持的表述格式：RJSON、JSON、HTML、XML。
## 资源层次
![](../../../../../../assets/images/root/spatialanalyst/datasets/planeProjection/sp_ds_planeProjection.png)
[
## HTTP 请求方法
...
| 名称 | 类型 | 含义 |
| --- | --- | --- |
| sourceDatasetFilter | [QueryParameter](../../../../../iServerJavadoc/com/supermap/services/components/commontypes/QueryParameter.html) | 过滤查询参数，包括属性过滤和ID过滤，只有满足条件的对象才参与获取平面投影。 |
| geometry | [Geometry3D](../../../../../iServerJavadoc/com/supermap/services/components/commontypes/Geometry3D.html) | 投影平面，目前暂时采用xy平面，传null即可。 |
...
#### 响应示例
对planeProjection资源：http://supermapiserver:8090/iserver/services/spatialAnalysis-BIM/restjsr/spatialanalyst/datasets/结构柱@8ExportModel/planeprojection.rjson，操作对象选择为数据集，执行 
POST 请求，请求参数如下：
{
...
HEAD 请求可以用来判断planeProjection资源是否存在，或者客户端是否有权限访问planeProjection资源。通过对加.<format>的 
URI 执行 HEAD 请求，还可以快速判断planeProjection 资源是否支持<format>格式的表述。
## 请参见
- [datasetPlaneProjectionResult](datasetPlaneProjectionResult.htm)，[dataset](../dataset.htm)
- [客户端构建 
